...
void fazerTeste(struct pessoa *pessoa) { // Função atribui resultado do teste
  ...
  if (pessoa->tipoTeste ==
      TESTE_NORMAL) { // Verifica qual o tipo de testes (Teste normal ou rapido)
  ...
  if (probabilidade(
          probabilidadeInconclusivo)) { // Verifica se o teste for inconclusivo
...
char *printTipoPessoa(
    struct pessoa *pessoa) { // Função para escrever o tipo de pessoa Medico
...
struct pessoa criaPessoa() { // Cria a pessoa
  ...
  p.precisaHospital =
      ...
      (!p.idoso &&
       probabilidade(configuracao.probabilidadeNaoIdosoPrecisaHospital));
...
void FilaEspera(struct pessoa *pessoa) {
  ...
  if (pessoa->centroTeste == 1) { // CENTRO TESTES 1
    if (centroTestes1.numeroPessoasEspera <
        configuracao
            .tamanhoFilaCentro1) { // Se o numero de pessoas na fila de espera
      ...
      if (pessoa->numeroPessoasAFrenteParaDesistir <
          centroTestes1
              .numeroPessoasEspera) { // Se o numero de pessoas na fila
        ...
        centroTestes1
            .numeroPessoasEspera++; //É aumentado o numero de pessoas em espera
        pthread_mutex_unlock(&mutexVariaveisCentros);
        sem_wait(&centroTestes1.filaEspera);
        pthread_mutex_lock(&mutexVariaveisSimulacao);
        tempoEspera =
            minutosDecorridos -
            pessoa->tempoChegadaFilaEspera; // Calculado e registado o tempo de
                                            // maximo de espera dessa pessoa
        pthread_mutex_unlock(&mutexVariaveisSimulacao);
        if (tempoEspera >
            pessoa->tempoMaximoEspera) { // Se passou muito tempo à espera, a
                                         // pessoa desiste
          pessoa->desistiu = TRUE;       // Pessoa desiste
          pthread_mutex_lock(&mutexVariaveisCentros);
          ...
          if (valorSemaforo <
              centroTestes1
                  .numeroPostosDisponiveis) { // Liberta uma pessoa da
          ...
          for (index = 0; index < configuracao.numeroPontosTestagemCentro1;
            ...
            if (tempoCooldownPontosTestagemCentro1[index] ==
                -1) // Verifica qual o posto testagem livre e começa-se o
                    // cooldown
            {
              ...
              tempoCooldownPontosTestagemCentro1[index] =
                  configuracao
                      .tempoCooldownPontosTestagem; //É atribuito o cooldown ao
                                                    // posto escolhido
              break;
            }
          }
          ...
          somaTemposEspera +=
              tempoEspera; //É somado o tempo de espera que essa pessoa esteve
    ...
    int numeroPessoasEsperaCentro2 =
        ...
        centroTestes2
            .numeroPessoasPrioritariasEspera; // O numero de pessoas em espera
                                              // do centro 2 é a soma do numero
                                              // de pessoas normais em espera
                                              // com o numero de pessoas
                                              // prioritarias em espera
    pthread_mutex_unlock(&mutexVariaveisCentros);
    if (numeroPessoasEsperaCentro2 <
        configuracao
            .tamanhoFilaCentro2) { // Se o numero de pessoas na fila de espera
                                   // for menor que o tamanho da fila avança
      tipoPessoa = printTipoPessoa(pessoa);
      printf("%s chegou a fila do centro 2.\n", tipoPessoa);
      free(tipoPessoa);
      if (pessoa->numeroPessoasAFrenteParaDesistir <
          numeroPessoasEsperaCentro2) { // Se o numero de pessoas na fila de
        ...
        if (pessoa->idoso) { // Verifica se é um idoso ou não
          ...
          if (!(centroTestes2.numeroPessoasPrioritariasEspera == 0 ||
                idososTestadosConsecutivamente >=
                    5)) { // Se o numero de pessoas na fila de espera
          ...
          pthread_mutex_lock(&mutexVariaveisCentros);
          centroTestes2
              .numeroPessoasNormalEspera++; // Aumenta o numero de pessoas da
        ...
        tempoEspera =
            minutosDecorridos -
            pessoa->tempoChegadaFilaEspera; //É calculado e registado o tempo de
                                            // maximo de espera dessa pessoa
        pthread_mutex_unlock(&mutexVariaveisSimulacao);
        if (tempoEspera >
            pessoa->tempoMaximoEspera) { // Se passou muito tempo à espera, a
...
void Pessoa(void *ptr) {
  ...
  while (TRUE) {
    ...
    if (!pessoa.desistiu) {
      ...
      if (pessoa.tipoTeste ==
          TESTE_RAPIDO) { // Se o tipo de teste for teste rapido
      ...
      fazerTeste(
          &pessoa); // Entra na função para saber qual o resultado do teste
      if (pessoa.estadoTeste == POSITIVO) { // Se o teste for positivo
        ...
        pessoa.numeroDiasDesdePositivo =
            0; // Coloca a 0 o numero de dias desde o ultimo caso positivo
        break;
        ...
        sem_post(&mutexVariaveisMonitor);
        pessoa.estado =
            SOBREVIVEU; // O estado da pessoa é colocado como sobrevivente
        break;
  ...
  if (pessoa.estadoTeste == POSITIVO) { // Se o teste for positivo
    ...
    if (pessoa.precisaHospital) { // Vai para o Hospital se houver camas livres
      ...
      if (numeroPacientesNoHospital <
          configuracao
              .tamanhoHospital) { // Se o numero de pacientes no hospital for
        // menor que o tamanho do hospital, a pessoa é
        ...
        for (index = 0; index < configuracao.tamanhoHospital; index++) {
          if (IDsDoentesNoHospital[index] == -1) {
            IDsDoentesNoHospital[index] =
                pessoa.id; // A pessoa é colocada numa vaga do hospital
            break;
          }
        }
        ...
        tipoPessoa = printTipoPessoa(&pessoa);
        printf(AMARELO
               "%s precisava de ajuda medica mas ficara em isolamento.\n" RESET,
               tipoPessoa);
        free(tipoPessoa);
      }
    }
  ...
  while (TRUE) {
    ...
    if (medico.estado == ISOLAMENTO) { // Se o medico estiver no isolamento
      while (TRUE) {
        usleep(tempoEsperaTeste);
        fazerTeste(
            &medico); // Entra na função para saber qual o resultado do teste
        if (medico.estadoTeste == POSITIVO) { // Se o teste for positivo
          ...
          casosPositivosTotal++;  // Aumenta o numero de casos positivos total
          sem_post(&mutexVariaveisMonitor);
          medico.numeroDiasDesdePositivo =
              0; // Coloca a 0 o numero de dias desde o ultimo caso positivo
          break;
          ...
          sem_post(&mutexVariaveisMonitor);
          medico.estado =
              SOBREVIVEU; // O estado da médico é colocado como sobrevivente
          break;
        } else {
          tipoPessoa = printTipoPessoa(&medico);
    ...
    if (medico.estadoTeste == POSITIVO) { // Se o teste for positivo
      medico.numeroDiasDesdePositivo =
          0; // Coloca a 0 o numero de dias desde o ultimo caso positivo
      if (medico
              .precisaHospital) { // Vai para o Hospital se houver camas livres
        pthread_mutex_lock(&mutexVariaveisHospital);
        if (numeroPacientesNoHospital <
            configuracao
                .tamanhoHospital) { // Se o numero de pacientes no hospital for
          ...
          for (index = 0; index < configuracao.tamanhoHospital; index++) {
            if (IDsDoentesNoHospital[index] == -1) {
              IDsDoentesNoHospital[index] =
                  medico.id; // O médico é colocada numa vaga do hospital
              break;
            }
          }
          sem_post(&semaforoMedicos);
    ...
    medico.estadoTeste =
        NAO_TESTOU; // O estado da médico é colocado como não testou
  }
}
...
void simulacao(char *filename) {
  ...
  for (index = 0; index < configuracao.tamanhoHospital;
       index++) { //É colocada a lista de doentes no hospital e a lista medicos
  ...
  sprintf(mensagem, "%d-%d-%d", configuracao.tamanhoHospital, 11,
  ...
  int numeroPessoasRecuperaramHospital = 0,
      ...
      numeroPessoasMorreramIsolamento = 0, numeroPessoasMorreramHospital = 0,
  ...
  if (configuracao.diasPicos[indexPicos] ==
      numeroDia) { // Se o dia atual for igual a um dos dias de pico as
    ...
    if (assinalarSemaforoPrioritario) {
      ...
      if (valorSemaforo < centroTestes2.numeroPostosDisponiveis) {
        sem_post(
            &centroTestes2.filaEsperaPrioritaria); // Liberta fila prioritaria
      }
    }
    pthread_mutex_unlock(&mutexVariaveisCentros);
    printf("---------------------------------------------INICIO "
  ...
  while (tempoDecorrido !=
         tempoLimite) { // Enquanto o tempo decorrido não for igual ao tempo
                        // limite a simulação continua
    auxTimeStamp = current_timestamp();
    if (auxTimeStamp !=
        timeStampAnterior) { // Se o tempo atual for diferente do tempo
      ...
      if (tempoDecorrido % MINUTO == 0) { // Passou um minuto na simulação e é
        ...
        strcat(mensagensAEnviar, mensagem);
        if (numeroPessoasTestadas >
            0) { // Se o numero de pessoas testadas for maior que 0, é enviada a
        ...
        for (index = 0; index < configuracao.numeroPontosTestagemCentro1 + 1;
             index++) {
          if (tempoCooldownPontosTestagemCentro1[index] ==
              0) { // Se o tempo de cooldown do posto for igual a 0, esse posto
            ...
            sem_post(&centroTestes1.filaEspera);
            tempoCooldownPontosTestagemCentro1[index]--;
          } else if (tempoCooldownPontosTestagemCentro1[index] >
                     0) { // Se o tempo de cooldown do posto for maior que 0, é
        ...
        for (index = 0; index < configuracao.numeroPontosTestagemCentro2 + 1;
             index++) {
          if (tempoCooldownPontosTestagemCentro2[index] ==
              0) { // Se o tempo de cooldown atinge os 0, o posto é colocado
                   // como disponivel
            printf(
                MAGENTA
            ...
            if (assinalarSemaforoPrioritario) {
              ...
              if (valorSemaforo < centroTestes2.numeroPostosDisponiveis) {
                sem_post(
                    &centroTestes2
                         .filaEsperaPrioritaria); // Liberta fila prioritaria
              }
            }
            tempoCooldownPontosTestagemCentro2[index]--; // Coloca o tempo de
                                                         // cooldown a -1;
          } else if (tempoCooldownPontosTestagemCentro2[index] >
                     0) { // Se o tempo de cooldown do posto for maior que 0, é
                          // feita a contagem decrescente do cooldown
            tempoCooldownPontosTestagemCentro2[index]--;
          }
        ...
        if (minutosDecorridos % (60 * 24) ==
            0) { // Se o minutos decorridos for igual a 1 dia
          printf("---------------------------------------------DIA "
                 "%d---------------------------------------------\n",
          ...
          numeroPessoasRecuperaramIsolamento = 0,
          numeroPessoasMorreramHospital = 0,
          numeroPessoasMorreramIsolamento = 0, numeroMedicosParaIsolamento = 0,
          numeroMedicosRecuperaram = 0;
          for (index = 0; index < configuracao.tamanhoHospital; index++) {
            libertarMedico =
                FALSE; // variavel que indica se medico pode ir para isolamento
            if (IDsDoentesNoHospital[index] != -1) { // Se há doente no hospital
              if (PessoasCriadas[IDsDoentesNoHospital[index]]
                ...
                if (pessoaMorreu) {
                  ...
                  PessoasCriadas[IDsDoentesNoHospital[index]]->estado =
                      MORREU; // O estado da pessoa passa para morreu
                  IDsDoentesNoHospital[index] =
                      -1; // A vaga do hospital passa a estar livre
                  libertarMedico = TRUE; // O medico deve ir para isolamento
                }
              ...
              if (IDsMedicosASerUsados[index] != -1 &&
                  libertarMedico) { // Se o médico tiver sido utilizado e já
                ...
                sem_post(&PessoasCriadas[IDsMedicosASerUsados[index]]
                              ->semaforoPessoa);
                IDsMedicosASerUsados[index] =
                    -1; // A posição que o medico estava a utilizar na lista de
            ...
            if (IDsDoentesNoHospital[index] !=
                -1) { // Se a pessoa não recuperou e não morreu, aumentar o
                      // contador
              PessoasCriadas[IDsDoentesNoHospital[index]]
                  ->numeroDiasDesdePositivo++; //É aumentado o numero de dias
          ...
          for (index = 0; index < idPessoaAtual; index++) {
            if (PessoasCriadas[index] != NULL &&
                ...
                PessoasCriadas[index]->estadoTeste ==
                    POSITIVO) { // Se a pessoa se encontra em isolamento com o
                                // teste positivo
              ...
              } else if (PessoasCriadas[index]->numeroDiasDesdePositivo ==
                         configuracao
                             .tempoCurar) { // Se já passou o tempo para curar
                tipoPessoa = printTipoPessoa(PessoasCriadas[index]);
                printf(AZUL "%s recuperou em isolamento.\n" RESET, tipoPessoa);
                free(tipoPessoa);
                PessoasCriadas[index]->estado =
                    SOBREVIVEU; // O estado da pessoa passa a ser sobreviveu
                PessoasCriadas[index]->estadoTeste =
                    NAO_TESTOU; // O estado do teste da pessoa passa a ser nao
                                // testou
                numeroPessoasRecuperaramIsolamento++; //É aumentado o numero de
                                                      // pessoas que recuperaram
                                                      // no isolamento
                numeroMedicosRecuperaram +=
                    PessoasCriadas[index]
                        ->medico; // Caso a pessoa seja médico (+1) é aumentado
          ...
          casosRecuperados +=
              ...
              numeroPessoasRecuperaramHospital; // O numero de casos recuperados
                                                // é a soma do numero de pessoas
                                                // que recuperaram no isolamento
                                                //, o numero de pessoas que
                                                // recuperaram no hostipal e o
                                                // numero de casos recuperados
                                                // anteriormente
          numeroPacientesNoHospital -=
              (numeroPessoasRecuperaramHospital +
               numeroPessoasMorreramHospital); // O numero de pacientes no
                                               // hostipal é a subtração do
                                               // numero de pacientes no
                                               // hospital anteriormente e a
                                               // soma do numero de pessoas que
                                               // recuperarm no hospital e o
                                               // numero de pessoas que morreram
                                               // no hospital
          pessoasEmIsolamento -=
              (numeroPessoasRecuperaramIsolamento +
               numeroPessoasMorreramIsolamento); // O numero de pessoas em
                                                 // isolamento é subtraçao do
                                                 // numero de pessoas em
                                                 // isolamento anteriomente e o
                                                 // numero de pessoas que
                                                 // recuperaram no isolamento e
                                                 // o numero de pessoas que
                                                 // morreram em isolamento
          pessoasEmIsolamento +=
              numeroMedicosParaIsolamento; // O numero de pessoas em isolamento
                                           // é a soma do numero de pessoas em
                                           // isolamento anteriormente  e o
                                           // numero de medicos para isolamento
          medicosDisponiveis +=
              numeroMedicosRecuperaram; // O numero de medicos disponiveis é a
                                        // soma de medicos disponiveis
                                        // anteriormente e o numero de medicos
                                        // que recuperaram
          casosEmEstudo +=
              numeroMedicosParaIsolamento; // O numero de casos em estudo é a
                                           // soma do numero de casos em estudo
                                           // anteriormente e o numero de
                                           // medicos para isolamento
          casosPositivosAtivos -=
              (numeroPessoasRecuperaramHospital +
               numeroPessoasMorreramHospital + numeroPessoasMorreramIsolamento +
               numeroPessoasRecuperaramIsolamento); // O numero de casos
                                                    // postivos ativos é
                                                    // subtração do numero de
                                                    // casos positivos ativos
                                                    // anteriormente e a soma do
                                                    // numero de pessoas que
                                                    // recuperaram no hospital,
                                                    // o numero de pessoas que
                                                    // morreram e o numero de
                                                    // pessoas que recuperaram
                                                    // no isolamento e o numero
                                                    // de pessoas que morreram
                                                    // em isolamento
          numeroMortos +=
              numeroPessoasMorreramHospital +
              numeroPessoasMorreramIsolamento; // O numero de mortos é a soma do
                                               // numero de mortos anteriomente
                                               // e o numero de pessoas que
                                               // morreram no hospital e em
                                               // isolamento
          sem_post(&mutexVariaveisMonitor);
          strcat(mensagensAEnviar, "/");
          ...
          if (configuracao.diasPicos[indexPicos] ==
            ...
            printf("---------------------------------------------INICIO "
                   "PICO--------------------------------------------\n");
                     ...
                     numeroDia) { // Se o dia atual for igual a um dos dias de
                                  // fim de pico as probablidades de a população
                                  // e os medicos darem positivo voltam ao
                                  // normal O tempo medio de chegadas volta ao
                                  // normal E o numero de postos por centro
                                  // voltam ao normal
            tempoMedioChegadaCentros *= 2;
            configuracao.probabilidadePopulacaoPositivo -= 0.1;
            configuracao.probabilidadeMedicoPositivo -= 0.1;
...
void iniciarSemaforosETrincos() {
  ...
  if (pthread_mutex_init(&mutexVariaveisCentros, NULL) !=
      0) { // Mensagem de erro
    printf("Inicializacao do trinco falhou.\n");
  }
  if (pthread_mutex_init(&mutexVariaveisSimulacao, NULL) !=
      0) { // Mensagem de erro
    printf("Inicializacao do trinco falhou.\n");
  }
  if (pthread_mutex_init(&mutexVariaveisHospital, NULL) !=
      0) { // Mensagem de erro
    printf("Inicializacao do trinco falhou.\n");
  }
