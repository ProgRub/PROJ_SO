...
int * Recebe_Mensagem(int sockfd)
{
	...
	sscanf(recvline,"%d %d %d %d %d %d %d %d %d ", &ID_Cliente, &Estado, &produto, &desistencias, &pac_min, &pac_max, &stock_prod, &empregados, &cliente_troca);
...
void Interpreta_Mensagem()
{
	...
	switch(Estado) //Evento
	{
		...
		printf("\n");	
		printf("Desistências=%d",desistencias);
		printf("\n");	
		printf("Empregados=%d",empregados);
		printf("\n");
...
void Menu_Principal()
{
	...
	switch(Escolha_utilizador)
	{
		...
		while(Simulacao_Em_Curso==false) //Se Nao Houver Nenhuma Simulacao Activa, Vai Comecar Uma Simulacao Nova
		{
